function U_dydt = MRG_diff(t,y,U_params,activ_params,flux_params,intracom_params,naxons,flux,activn)
    ...
    if naxons > 1
        ...
        for n = 1:naxons-1
            ...
            diff_U(1,pas1:pas10) = -intracom_params.ext' .* (U_vars(1,pas1:pas10)) + intracom_params.ext' .* (U_vars(2,pas1:pas10) - U_vars(1,pas1:pas10)+80);
            diff_U(2,pas1:pas10) = -intracom_params.int' .* (U_vars(2,pas1:pas10) - U_vars(1,pas1:pas10)+80);% + diff_U(1,pas1:pas10);
            if n+1 == activn
        ...
        if flux == 1
            ...
            flux_ext = ((padU_ext(1:end-2)-U_ext) ./ ext_taus2(1:end-1)) + ((padU_ext(3:end)-U_ext)./ext_taus2(2:end));%+((padU_diff(1:end-2)-U_diff) ./ ext_taus1(1:end-1)) + ((padU_diff(3:end)-U_diff)./ext_taus1(2:end))
            flux_ext(1) = ((padU_ext(3)-U_ext(1))./ext_taus2(2));%+((padU_diff(3)-U_diff(1))./ext_taus1(2));
            flux_ext(end) = ((padU_ext(end-2)-U_ext(end)) ./ ext_taus2(end-1));%+((padU_diff(end-2)-U_diff(end)) ./ ext_taus1(end-1));
            ...
            flux_int(1) = flux_ext(1) + (padU_diff(3)-U_diff(1))./int_taus(2);
            flux_int(end) = flux_ext(end) + (padU_diff(end-2)-U_diff(end))./int_taus(end-1);
